...
export const runtime = new (class implements PlatformRuntime {
    ...
    getFeatures() {
        ...
        window.utools.getFeatures()
            .filter(item => item.code.includes("sectool-") && item.code.includes("-customize"))
            .forEach(item => {
                    ...
                    cmds: [
                        ...(
                            new Set([
                                    tool.name,
                                    feature.name,
                                    tool.isSimple() ? `sectool-${tool.name}` : `sectool-${tool.name}-${feature.name}`,
                                    $t(`tool_${tool.name}`),
    ...
    setFeatures(features: FeatureItem[]) {
        ...
        window.utools.getFeatures().forEach(item => {
            if (item.code.includes("sectool-") && item.code.includes("-customize")) {
        ...
        features.forEach(({ feature, cmds }) => {
            if (cmds.length > 0) {
                setFeatures({
                    "code": `sectool-${feature.getKey()}-customize`,
