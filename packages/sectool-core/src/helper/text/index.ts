...
class Text {
    ...
    static fromHex(str: string, { preserve_line_breaks = false }: { preserve_line_breaks?: boolean } = {}): Text {
        return Text.fromBuffer(
            Buffer.from(
                str
                    .replace(/\\x/g, '') // 兼容 Unicode Escape 格式
                    .replace(/ /g, "") // 过滤空格
                    .replace(/\n/g, preserve_line_breaks ? "0a" : "") // 换行符处理
                    .trim(),
                "hex",
            ),
        );
    ...
    toDown() {
        const objectUrl = window.URL.createObjectURL(new Blob([this.uint.uint8Array as unknown as BlobPart], { type: this.mime() }));
    ...
    name() {
        ...
        return `sectool-${dayjs().format("YYYY-MM-DD-HH-mm-ss")}${this.extension()}`;
    }
    ...
    setExtension(extension: string) {
        this.fileName = `sectool-${dayjs().format("YYYY-MM-DD-HH-mm-ss")}${extension}`;
        return this;
    }
