<template>
    <HeightResize v-slot="{small,large}" ignore :append="['.sectool-page-option']" :reduce="10">
        <Align direction="vertical">
            <TextInput
                ...
                encoding
            />
            <Input class="sectool-page-option" v-model="action.current.option.key" label="key">
                <template #suffix>
...
const output = $computed<Text>(() => {
    ...
    try {
        ...
        return Text.fromBase64(rc4.encrypt(action.current.input.text.toBase64(), action.current.option))
    } catch (e) {
