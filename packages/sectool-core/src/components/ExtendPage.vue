<template>
    <Teleport to="#sectool-append">
        <Transition name="sectool-extend-page">
            <div class="sectool-extend-page" :style="style" v-if="show" v-bind="$attrs">
                <slot></slot>
            </div>
        </Transition>
        <Transition name="sectool-extend-page">
            <div class="sectool-extend-page-close" v-if="show">
...
let top = $ref(document.querySelector<HTMLElement>('.sectool-header')?.offsetHeight || 33)
let bottom = $ref(document.querySelector<HTMLElement>('.sectool-bottom')?.offsetHeight || 33)
...
const resize = () => {
    top = document.querySelector<HTMLElement>('.sectool-header')?.offsetHeight || 33
    bottom = document.querySelector<HTMLElement>('.sectool-bottom')?.offsetHeight || 33
}
...
.sectool-extend-page-enter-active, .sectool-extend-page-leave-active {
    transition: all 0.3s ease-in-out;
}

.sectool-extend-page-enter-from, .sectool-extend-page-leave-to {
