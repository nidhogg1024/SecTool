<template>
    <Display
        ...
        class="sectool-code-editor"
        :class="props.disableBorder ? ['sectool-code-editor-disable-border'] : []"
...
const create = async (element: HTMLElement) => {
    ...
    monacoInit({
        ...
        contextMenu.setHandle("sectool_line_wrapping", (_ed, _id, result) => (lineWrapping.value = result));
        contextMenu.setHandle("sectool_line_number", (_ed, _id, result) => (lineNumbers.value = result));
...
.sectool-code-editor-disable-border.sectool-code-editor {
    border: none;
}
</style>
