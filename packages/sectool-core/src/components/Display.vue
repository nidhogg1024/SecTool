<template>
    <template v-if="is">
        <div class="sectool-display" v-bind="$attrs">
            <slot></slot>
            <div :style="extraStyle" style="display: inline-flex">
                <span
                    ref="extra"
                    class="sectool-display-extra" :class="[
                        `sectool-display-extra-${position}`,
                        position.includes('right') ? 'sectool-display-extra-right' : '',
                        position.includes('left') ? 'sectool-display-extra-left' : '',
                        `sectool-display-extra-${isShowExtra ? 'show' : 'hide'}`,
                    ]"
                >
                    <template v-if="isToggle && position.includes('right')">
                        <Tooltip
                            ...
                            "
                        >
                            <div class="sectool-display-toggle" @click="extraToggle">
...
.sectool-display .sectool-display-extra {
    display: inline-flex;
    align-items: center;
}

.sectool-display-toggle {
...
.sectool-display .sectool-display-extra-right .sectool-display-toggle {
    justify-content: right;
}

.sectool-display .sectool-display-extra-left .sectool-display-toggle {
    justify-content: left;
}

.sectool-display
    :is(.sectool-display-extra-hide.sectool-display-extra-right, .sectool-display-extra-show.sectool-display-extra-left)
    .sectool-display-toggle
    .sectool-icon {
    transform: rotate(180deg);
}

.sectool-display .sectool-display-extra-hide .sectool-display-toggle {
    width: unset;
    height: unset;
}
</style>
